...
**2 — Le rail bancaire propre (simplifié 20/07).** **Wio** (Business Essential + Perso : orchestration + ta vie + **1 mois de tampon instantané**) + **IBKR** (coussin ET patrimoine, un seul nouveau compte, **en ligne**). **Un seul compte indispensable à ouvrir : IBKR.** **FAB (banque physique) reste OPTIONNEL — mais il a DEUX vrais rôles, pas un** : (a) accès **AED local instantané** (loyer, dépôt, guichet) ; (b) **rail de bankabilité** si le crédit immo 2027 est sérieux — y faire atterrir ton salaire documenté + y tenir une carte payée à l'heure construit la **relation prêteur + l'historique AECB** qu'un underwriter regarde (Emirates NBD = équivalent). Le crédit vient de la **trace** (revenu documenté + crédit propre + apport + DBR ≤ 50 %), *pas du logo* — mais la trace, elle, se construit **maintenant**. Wio = quotidien (Spaces, Autopilot, export CSV → Google Sheet) ; IBKR tu le regardes 1×/mois. Détail + convergence « bankable = fin 2027 » : [[Kill-list (NON, pas maintenant)|kill-list, ligne 7]] et [[Architecture bancaire Profit First (comptes, coffres, crypto, compta)]].
