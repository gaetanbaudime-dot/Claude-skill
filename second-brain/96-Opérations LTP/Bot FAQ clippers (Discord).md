...
## Serveur fermé aux candidats (14/09, soir)

Gaëtan : « je veux plus personne qui arrive sur le Discord, je veux que les clippers actifs dessus ». Le tunnel candidat (formation, quiz, test 48 h, rendu) sort de Discord : trois Apps Script envoient les e-mails et postent au bot (`QUIZ_OK` avec le numéro WhatsApp, `TEST_RENDU`), le manager juge le test rendu et tape `!inviter Prénom` (invitation personnelle, 7 jours, une personne, message WhatsApp prêt à coller), et à l'arrivée le bot relie, valide et enchaîne contrat ou J'ACCEPTE tout seul. Serveur fermé (`!fermer`), un arrivant sans invitation validée est raccompagné (MP + expulsion) ; `!purge-candidats` vide le stock de non-signés en protégeant rôles, signés et parcours en cours. Analyse complète et carte cible : [[Architecture Discord - simple au quotidien (14 septembre 2026)]] (v2).
