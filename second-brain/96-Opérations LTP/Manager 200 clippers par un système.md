...
Le pipeline d'exceptions : **bot** (collecte, conformité, tracking, relances) → **manager** (exceptions qualité/humaines) → **lead senior** (exceptions de règles) → **toi** (exceptions de *système* : une règle manque ou deux se contredisent). Le transfert concret de cette couche « manager » vit dans **[[Les 6 Loom de délégation manager (passation été)]]** (Opus Clip + pilotage `!pipeline` + validation `!test-ok` + contrat/onboarding + paie sacrée + qualité/blocages) — la passation prête à tourner avant le départ. Ton seul travail hebdo non délégable (60 min) : lire l'issue log ([[Journal de coaching]]), modifier les règles, décider les promotions. Les règles vivent dans UNE source versionnée (le kit + les fiches) — le Discord annonce, il ne fait jamais foi. Test de Gerber avant chaque délégation : *« ce rôle marcherait-il tenu par une personne au niveau de compétence minimal possible ? »* — sinon la SOP n'est pas finie.
