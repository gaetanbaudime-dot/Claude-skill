...
- **Le signal terrain du 28/08 midi (« ça marchait pas ») — à préciser, il change le diagnostic** : ① si c'est la **résa/le site en panne** → preuve de douleur, à capturer (photo/capture d'écran) : c'est l'ouverture parfaite du pitch ; ② si c'est la **salle vide** → red flag sur le fonds de commerce : un resto qui ne remplit pas fin août n'a pas un problème d'outil (nuance : le 28/08 est déjà la fin de saison — la 2e quinzaine d'août retombe, la clientèle rentre pour la rentrée ; comparer à un midi de juillet avant de conclure).
