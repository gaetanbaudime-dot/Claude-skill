...
## À câbler AVANT le départ (~26/07) — surtout en ligne

*Bonne nouvelle : presque tout se fait à distance. **FAB = app** (Emirates ID + liveness) ; **Binance corporate = KYB en ligne** (mais le maillon incertain, nexus adulte). Le vrai « pourquoi le faire depuis Dubaï maintenant » = recevoir la **carte physique FAB** à ton adresse UAE avant de partir + débloquer un éventuel **blocage KYB** tant que tu es sur place.*

- [ ] **Ouvrir le compte perso — FAB, EN LIGNE (app FAB Mobile)** : Emirates ID + selfie/liveness → compte actif sous **24-48 h**, **carte de débit numérique immédiate**, physique livrée **~2 j** à ton adresse UAE. **Fais-le maintenant, depuis Dubaï**, pour recevoir la carte physique avant le 26/07. ⚠️ Le **compte + débit** est instantané ; la **carte de CRÉDIT (AECB)** est un produit **séparé** qui demande souvent un historique de salaire → sinon demande une **carte sécurisée** (adossée à un dépôt), même effet AECB. iSave + DAT s'ouvrent aussi dans l'app. Profil self-employed → l'app peut réclamer des docs / une brève étape agence (`to-verify`). *(Emirates NBD s'ouvre pareil en app si tu veux comparer.)*
- [x] **Ouvrir le compte IBKR individuel** — fait le **21/07** : compte **Cash** (zéro levier), permissions **Stocks uniquement** (Options + Prediction Markets coupés), **SYEP refusé** (prêt de titres = perte de protection + revenu dérisoire sur ETF larges), devise de base **EUR**, **W-8BEN** résidence **UAE sans traité** (les UCITS irlandais captent le 15 % au niveau du fonds, indépendamment du W-8BEN), citoyenneté **France** (≠ résidence fiscale, aucune conséquence FR). Identité/adresse vérifiées (Emirates ID + liveness) → approbation attendue ~24 h. **Funding en attente de FAB** : le 1er dépôt doit venir d'un compte **à ton nom (FAB)**, jamais Wio/LTP (règle anti-tiers, IBKR l'a confirmée à l'écran).
