...
- **« Pull failed : your local changes would be overwritten »** (vécu le 13/07) : Obsidian a des modifications locales non commitées (souvent le panneau Propriétés qui réécrit le YAML en affichant une page). Rien n'est perdu — le fix : palette de commandes → **« Obsidian Git: Commit all changes »** puis **« Pull »**. Prévention : utiliser **« Commit-and-sync »** (commit + pull + push en un geste) plutôt que Pull seul, et/ou activer l'auto-commit du plugin (intervalle 5-10 min).
