...
async def commande_admin(message, texte: str) -> bool:
    ...
    # ---- !tests : qui est en test, qui a laissé expirer, et relance en un geste ----
    # `!pipeline` donne les compteurs mais aucun nom : impossible de savoir QUI relancer.
    # `!tests` nomme, `!tests relancer` renvoie un test de 48 h aux expirés.
    if texte.startswith("!tests"):
        ...
        en_cours, expires, rendus = [], [], []
        for uid, info in etats.items():
            etat = info.get("etat")
            if etat not in ("test_envoye", "test_expire", "test_rendu"):
                continue
            m = trouver(uid)
            nom = m.display_name if m else f"parti du serveur (id {uid})"
            if etat == "test_envoye":
                reste = datetime.fromisoformat(info["echeance"]) - maintenant
                h = int(reste.total_seconds() // 3600)
                en_cours.append((nom, m, f"{h} h restantes" if h > 0 else "échéance dépassée, clôture imminente"))
            elif etat == "test_rendu":
                rendus.append((nom, m, "à reviewer"))
            else:
                exp = str(info.get("echeance", ""))[:10]
                rt = str(info.get("retest", ""))[:10]
                expires.append((nom, m, f"expiré le {exp}" + (f", re-test ouvert le {rt}" if rt else ""), uid, info))

        lignes = ["🧪 **État des tests**", ""]
        for titre, groupe in (("🧪 **En cours**", en_cours), ("📥 **Rendus, à reviewer**", rendus)):
            lignes.append(titre)
            lignes += [f"· {n} — {d}" for n, _m, d in groupe] or ["_aucun_"]
            lignes.append("")
        lignes.append("⌛ **Expirés**")
        lignes += [f"· {n} — {d}" for n, _m, d, _u, _i in expires] or ["_aucun_"]
        if expires and not relancer:
            partis = sum(1 for _n, m, _d, _u, _i in expires if m is None)
            lignes += ["", f"Pour leur renvoyer un test de 48 h : `!tests relancer`"]
            if partis:
                lignes.append(f"⚠️ {partis} d'entre eux ne sont plus sur le serveur (purge ou départ) — "
                              f"ils seront ignorés.")
        await envoyer_long(message, lignes)
        if not relancer or not expires:
            return True

        relances, ignores = [], []
        for nom, m, _d, _uid, info in expires:
            if m is None:
                ignores.append(nom)
                continue
            ...
            ok = await envoyer_test_candidat(m, info.get("score_quiz", ""))
            relances.append(f"{nom}{'' if ok else ' (MP fermés — à relancer à la main)'}")
